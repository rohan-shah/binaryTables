#include "withoutReplacementSingleStepImpl.h"
#include "conditionalPoissonSequential.h"
#include "samplingBase.h"
#include "GayleRyserTest.h"
namespace binaryTables
{
	withoutReplacementSingleStepSample::withoutReplacementSingleStepSample(withoutReplacementSingleStepSample&& other)
		: columnSum(other.columnSum), productInclusionProbabilities(std::move(other.productInclusionProbabilities)), totalRemaining(other.totalRemaining), expNormalisingConstants(other.expNormalisingConstants), expExponentialParameters(other.expExponentialParameters), skipped(other.skipped), nRemainingZeros(other.nRemainingZeros), nRemainingDeterministic(other.nRemainingDeterministic), deterministicInclusion(std::move(other.deterministicInclusion))
	{
	}
	withoutReplacementSingleStepSample& withoutReplacementSingleStepSample::operator=(withoutReplacementSingleStepSample&& other)
	{
	...
	void withoutReplacementSingleStep(withoutReplacementSingleStepArgs& args)
	{
		...
		std::vector<withoutReplacementSingleStepSample>& samples = args.samples;
		std::vector<withoutReplacementSingleStepSample>& newSamples = args.newSamples;
...
/*		if(n < 2)
		...
		if(initialRowSums[0] == 0)
		{
			...
			samples.push_back(withoutReplacementSingleStepSample(0, 1, totalOnes, initialExpNormalisingConstantData, initialExpExponentialParameters));
			samples[0].skipped = 1;
		}
		else if(initialRowSums[0] == (int)nColumns)
		{
			std::copy(initialRowSums.begin(), initialRowSums.end(), sampleRowSums.begin());
			samples.push_back(withoutReplacementSingleStepSample(1, 1, totalOnes-1, initialExpNormalisingConstantData, initialExpExponentialParameters));
			sampleRowSums[0]--;
			samples[0].skipped = 1;
		}
		else
		...
		for(; column < (int)nColumns; column++)
		{
			...
			for(; row < (int)nRows; row++)
			{
				...
				if(choicesUp.size() + choicesDown.size() <= n)
				{
					...
					for(std::size_t i = 0; i < choicesUp.size(); i++)
					{
						...
						withoutReplacementSingleStepSample& parentSample = samples[choicesUp[i]];
						...
						newSamples.push_back(withoutReplacementSingleStepSample(parentSample.columnSum + 1, parentSample.productInclusionProbabilities, parentSample.totalRemaining - 1, parentSample.expNormalisingConstants, parentSample.expExponentialParameters));
					...
					for(std::size_t i = 0; i < choicesDown.size(); i++)
					{
						...
						withoutReplacementSingleStepSample& parentSample = samples[parentIndex];
						std::copy(sampleRowSums.begin() + nRows * parentIndex, sampleRowSums.begin() + nRows * (parentIndex+1), newSampleRowSums.begin() + outputCounter * nRows);
						...
						newSamples.push_back(withoutReplacementSingleStepSample(parentSample.columnSum, parentSample.productInclusionProbabilities, parentSample.totalRemaining, parentSample.expNormalisingConstants, parentSample.expExponentialParameters));
						newSamples.back().skipped = newSkipped;
						newSamples.back().nRemainingZeros = parentSample.nRemainingZeros;
				...
				else
				{
...
#endif
						args.samplingArgs.weights.push_back(selectionProb);
					}
					for(std::size_t i = 0; i < choicesDown.size(); i++)
					{
						int parentIndex = choicesDown[i];
						...
						if(selectionProb.convert_to<double>() > 1) throw std::runtime_error("Internal error");
#endif
						args.samplingArgs.weights.push_back(1 - selectionProb);
					}
					...
					for(std::size_t i = 0; i < n; i++)
					{
						...
						if(args.samplingArgs.indices[i] >= (int)choicesUp.size())
						{
							...
							newSamples.push_back(withoutReplacementSingleStepSample(parentSample.columnSum, parentSample.productInclusionProbabilities * inclusionProbability, parentSample.totalRemaining, parentSample.expNormalisingConstants, parentSample.expExponentialParameters));
						...
						else
						{
							...
							newSamples.push_back(withoutReplacementSingleStepSample(parentSample.columnSum+1, parentSample.productInclusionProbabilities * inclusionProbability, parentSample.totalRemaining-1, parentSample.expNormalisingConstants, parentSample.expExponentialParameters));
							newSamples.back().skipped = parentSample.skipped;
							newSamples.back().nRemainingZeros = parentSample.nRemainingZeros;
							newSamples.back().nRemainingDeterministic = parentSample.nRemainingDeterministic;
							if(sampleRowSums[nRows * parentIndex + row] == (int)nColumns - column || parentSample.deterministicInclusion[row])
			...
			if(column != (int)nColumns - 1)
			{
				for(std::size_t i = 0; i < samples.size(); i++)
				{
					...
					try
					{
						withoutReplacementSingleStepSample& currentSample = samples[i];
